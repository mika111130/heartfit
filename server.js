...
// ── API 키 ──
const AIR_API_URL = 'https://apis.data.go.kr/B552584/ArpltnInforInqireSvc/getMsrstnAcctoRltmMesureDnsty';
const NEARBY_STATION_URL = 'https://apis.data.go.kr/B552584/MsrstnInfoInqireSvc/getNearbyMsrstnList';
const RAW_KEY = process.env.AIR_API_KEY || '';

if (!RAW_KEY) {
  console.warn('[경고] AIR_API_KEY 환경변수가 설정되지 않았습니다. 대기질/기상 API 호출이 실패합니다.');
}
...
async function findNearbyStation(lat, lon) {
  try {
    ...
    const response = await axios.get(`${NEARBY_STATION_URL}?serviceKey=${encodeURIComponent(RAW_KEY)}`, {
      params: {
        returnType: 'json',
        tmX: tmX,
        tmY: tmY,
    ...
    if (items && items.length > 0) {
      console.log(`[측정소 탐색 성공] 가장 가까운 측정소: ${items[0].stationName} (${items[0].tm}km)`);
...
async function getWeatherData(lat, lon) {
  try {
    ...
    console.log(`[기상청 API 요청] 격자: (${nx}, ${ny}), 발표일시: ${baseDate} ${baseTime}`);

    const response = await axios.get(`${KMA_API_URL}?serviceKey=${encodeURIComponent(KMA_KEY)}`, {
      params: {
        pageNo: '1',
        numOfRows: '10',
        dataType: 'JSON',
    ...
    let temp = null, humidity = null, windSpeed = null;
    ...
    console.log(`[기상청 데이터 성공] 기온: ${temp}°C, 습도: ${humidity}%, 풍속: ${windSpeed}m/s`);
...
function calcFeelsLike(temp, humidity, windSpeed) {
  ...
  if (temp >= 27 && humidity !== null) {
    const T = temp;
    const R = humidity;
    const hi = -8.7847 + 1.6114 * T + 2.3385 * R / 100 * 10 
               - 0.1461 * T * R / 100 * 10;
  ...
  if (temp <= 10 && windSpeed !== null && windSpeed >= 1.3) {
    const wc = 13.12 + 0.6215 * temp - 11.37 * Math.pow(windSpeed * 3.6, 0.16) 
...
function assessWeatherRisk(temp, humidity, feelsLike) {
  if (temp === null) return { level: 'unknown', message: '기상 데이터를 가져올 수 없습니다.' };
  
  // 극한 고온 (체감온도 35도 이상)
  if (feelsLike >= 35) return { level: 'danger', message: `체감온도 ${feelsLike}°C — 열사병 위험. 야외 활동을 금지합니다.` };
  // 고온 주의 (체감온도 31도 이상)
  if (feelsLike >= 31) return { level: 'warning', message: `체감온도 ${feelsLike}°C — 고온 주의. 활동 시간을 단축하세요.` };
  // 극한 저온 (체감온도 -10도 이하)
  if (feelsLike <= -10) return { level: 'danger', message: `체감온도 ${feelsLike}°C — 동상·저체온 위험. 실내 활동으로 대체하세요.` };
  // 저온 주의 (체감온도 0도 이하)
  if (feelsLike <= 0) return { level: 'warning', message: `체감온도 ${feelsLike}°C — 한파 주의. 방한 후 짧은 활동을 권장합니다.` };
  // 높은 습도 (85% 이상 + 기온 28도 이상)
  if (humidity >= 85 && temp >= 28) return { level: 'warning', message: `습도 ${humidity}%로 열 발산이 어렵습니다. 수분 섭취를 늘리세요.` };
...
app.get('/api/health-status', async (req, res) => {
  try {
    ...
    // 2) 에어코리아 대기오염 데이터
    let pm10 = 25, pm25 = 12, caiGrade = '1';
    let airDataSuccess = false;
    try {
      console.log(`[에어코리아 API 요청] 측정소: ${stationName}`);
      const airResponse = await axios.get(`${AIR_API_URL}?serviceKey=${encodeURIComponent(RAW_KEY)}`, {
        params: {
          returnType: 'json',
          numOfRows: '1',
          pageNo: '1',
      ...
      console.log(`[대기오염 데이터 성공] PM10: ${pm10}㎍/㎥, PM2.5: ${pm25}㎍/㎥, 등급: ${caiGrade}`);
    } catch (airErr) {
      console.log(`[에어코리아 API 실패] ${airErr.message} → 기본값 사용`);
    }

    // 4) 체감온도 계산
    const feelsLike = calcFeelsLike(weather.temp, weather.humidity, weather.windSpeed);
    ...
    const overallLevel = levels[weatherRisk.level] >= levels[airStatus] ? weatherRisk.level : airStatus;
    ...
    // 8) 추천 골든타임 동적 산출
    const goldenTimeInfo = calculateGoldenTime(temp, feelsLike, weatherRisk.level);
    ...
    const responseData = {
      ...
      windSpeed: weather.windSpeed !== null ? weather.windSpeed.toFixed(1) : '-',
    ...
    console.log(`[종합 판단] ${badge} — 기상: ${weatherRisk.level}, 대기: ${airStatus}`);
